...
static int s_mqtt5_utf8_encoded_string_test(struct aws_allocator *allocator, void *ctx) {
    ...
    for (size_t i = 0; i < AWS_ARRAY_SIZE(s_valid_utf8_examples); ++i) {
        ...
        ASSERT_TRUE(aws_decode_utf8(example.text, aws_mqtt_utf8_validator, NULL));
    }
    ...
    ASSERT_TRUE(aws_decode_utf8(aws_byte_cursor_from_buf(&all_good_text),aws_mqtt_utf8_validator, NULL)));
    ...
    for (size_t i = 0; i < AWS_ARRAY_SIZE(s_illegal_utf8_examples); ++i) {
        ...
        ASSERT_FALSE(aws_decode_utf8(example.text, aws_mqtt_utf8_validator, NULL));
    }
}
