...
#define AWS_MQTT5TO3_OVERSIZED_SUBACK_REASON_CODE_COUNT 200000

static int s_mqtt5_mock_server_send_packet_streaming(
    struct aws_mqtt5_server_mock_connection_context *connection,
    enum aws_mqtt5_packet_type packet_type,
    void *packet) {

    if (aws_mqtt5_encoder_append_packet_encoding(&connection->encoder, packet_type, packet)) {
        return AWS_OP_ERR;
    }
    ...
    while (result == AWS_MQTT5_ER_OUT_OF_ROOM) {
        struct aws_io_message *message =
            aws_channel_acquire_message_from_pool(connection->slot->channel, AWS_IO_MESSAGE_APPLICATION_DATA, 4096);
        if (message == NULL) {
            return AWS_OP_ERR;
        }

        result = aws_mqtt5_encoder_encode_to_buffer(&connection->encoder, &message->message_data);
...
static int s_mqtt5_mock_server_handle_subscribe_suback_oversized(
    void *packet,
    struct aws_mqtt5_server_mock_connection_context *connection,
    void *user_data) {
    ...
    size_t reason_code_count = AWS_MQTT5TO3_OVERSIZED_SUBACK_REASON_CODE_COUNT;

    enum aws_mqtt5_suback_reason_code *mqtt5_suback_codes =
        aws_mem_calloc(connection->allocator, reason_code_count, sizeof(enum aws_mqtt5_suback_reason_code));
    for (size_t i = 0; i < reason_code_count; ++i) {
        mqtt5_suback_codes[i] = AWS_MQTT5_SARC_GRANTED_QOS_1;
    }
    ...
    int result = s_mqtt5_mock_server_send_packet_streaming(connection, AWS_MQTT5_PT_SUBACK, &suback_view);

    aws_mem_release(connection->allocator, mqtt5_suback_codes);

    return result;
}

static int s_mqtt5to3_adapter_subscribe_multi_oversized_suback_fn(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    ...
    test_options.server_function_table.packet_handlers[AWS_MQTT5_PT_SUBSCRIBE] =
        s_mqtt5_mock_server_handle_subscribe_suback_oversized;
...
AWS_TEST_CASE(mqtt5to3_adapter_subscribe_multi_oversized_suback, s_mqtt5to3_adapter_subscribe_multi_oversized_suback_fn)
