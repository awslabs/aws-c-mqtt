...
static void s_on_connection_resumed(struct aws_mqtt_client_connection *connection, enum aws_mqtt_connect_return_code return_code,
                                    bool session_present, void *userdata) {
    (void)connection;
    (void)return_code;
    struct mqtt_connection_state_test *state_test_data = userdata;
...
static void s_wait_for_reconnect_to_complete(struct mqtt_connection_state_test *state_test_data) {
    aws_mutex_lock(&state_test_data->lock);
    aws_condition_variable_wait_pred(&state_test_data->cvar, &state_test_data->lock, s_is_connection_resumed, state_test_data);
    aws_mutex_unlock(&state_test_data->lock);
}
...
static int s_setup_mqtt_server_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    aws_mqtt_client_connection_set_connection_interruption_handlers(state_test_data->mqtt_connection, s_on_connection_interupted, state_test_data, s_on_connection_resumed, state_test_data);
...
static void s_wait_for_publish(struct mqtt_connection_state_test *state_test_data) {
    aws_mutex_lock(&state_test_data->lock);
    aws_condition_variable_wait_pred(&state_test_data->cvar, &state_test_data->lock, s_is_publish_received, state_test_data);
...
static void s_wait_for_subscribe_to_complete(struct mqtt_connection_state_test *state_test_data) {
    aws_mutex_lock(&state_test_data->lock);
    aws_condition_variable_wait_pred(&state_test_data->cvar, &state_test_data->lock, s_is_subscribe_completed, state_test_data);
    aws_mutex_unlock(&state_test_data->lock);
}
...
static int s_test_mqtt_connect_disconnect_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    struct aws_array_list *received_messages = s_mqtt_mock_server_get_received_messages(state_test_data->test_channel_handler);
    ...
    ASSERT_BIN_ARRAYS_EQUALS(connection_options.client_id.ptr, connection_options.client_id.len,
            connect_packet.client_identifier.ptr, connect_packet.client_identifier.len);
...
AWS_TEST_CASE_FIXTURE(mqtt_connection_interupted, s_setup_mqtt_server_fn,
                      s_test_mqtt_connection_interupted_fn, s_clean_up_mqtt_server_fn, &test_data)
...
AWS_TEST_CASE_FIXTURE(mqtt_connection_timeout, s_setup_mqtt_server_fn,
                      s_test_mqtt_connection_timeout_fn, s_clean_up_mqtt_server_fn, &test_data)
...
static int s_test_mqtt_subscribe_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_TRUE(aws_mqtt_client_connection_subscribe(state_test_data->mqtt_connection, &sub_topic, AWS_MQTT_QOS_AT_LEAST_ONCE,
                                                     s_on_publish_received, state_test_data, NULL, s_on_suback, state_test_data) > 0);
    ...
    ASSERT_SUCCESS(s_mqtt_mock_server_send_publish(state_test_data->test_channel_handler, &sub_topic, &payload_1, AWS_MQTT_QOS_AT_LEAST_ONCE));
    struct aws_byte_cursor payload_2 = aws_byte_cursor_from_c_str("Test Message 2");
    ASSERT_SUCCESS(s_mqtt_mock_server_send_publish(state_test_data->test_channel_handler, &sub_topic, &payload_2, AWS_MQTT_QOS_AT_LEAST_ONCE));
...
AWS_TEST_CASE_FIXTURE(mqtt_connect_subscribe, s_setup_mqtt_server_fn,
                      s_test_mqtt_subscribe_fn, s_clean_up_mqtt_server_fn, &test_data)
