...
static int s_test_mqtt_connection_publish_after_disconnect_fn(struct aws_allocator *allocator, void *ctx) {
    (void)allocator;
    struct mqtt_connection_state_test *state_test_data = ctx;
    ...
    // try to sneak in a publish right after
    ASSERT_INT_EQUALS(
        0,
        aws_mqtt_client_connection_publish(
            state_test_data->mqtt_connection,
    ...
    ASSERT_SUCCESS(mqtt_mock_server_decode_packets(state_test_data->mock_server));
    size_t packets_count = mqtt_mock_server_decoded_packets_count(state_test_data->mock_server);
    ASSERT_INT_EQUALS(2, packets_count);
...
AWS_TEST_CASE_FIXTURE(
    mqtt_connection_publish_after_disconnect,
    s_setup_mqtt_server_fn,
    s_test_mqtt_connection_publish_after_disconnect_fn,
    s_clean_up_mqtt_server_fn,
    &test_data)
