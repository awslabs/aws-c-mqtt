...
static int s_aws_mqtt5_user_property_set_validate(
    ...
    for (size_t i = 0; i < property_count; ++i) {
        ...
        if (!aws_decode_utf8(property->name, aws_mqtt_utf8_validator, NULL)) {
            AWS_LOGF_ERROR(
        ...
        if (!aws_decode_utf8(property->value, aws_mqtt_utf8_validator, NULL)) {
            AWS_LOGF_ERROR(
                AWS_LS_MQTT5_GENERAL,
...
int aws_mqtt5_packet_connect_view_validate(const struct aws_mqtt5_packet_connect_view *connect_options) {
    ...
    if (!aws_decode_utf8(connect_options->client_id, aws_mqtt_utf8_validator, NULL)) {
        AWS_LOGF_ERROR(
            AWS_LS_MQTT5_GENERAL,
    ...
    if (connect_options->username != NULL) {
        ...
        if (!aws_decode_utf8(*connect_options->username, aws_mqtt_utf8_validator, NULL)) {
            AWS_LOGF_ERROR(
                AWS_LS_MQTT5_GENERAL,
...
int aws_mqtt5_packet_disconnect_view_validate(const struct aws_mqtt5_packet_disconnect_view *disconnect_view) {
    ...
    if (disconnect_view->reason_string != NULL) {
        ...
        if (!(aws_decode_utf8(*disconnect_view->reason_string, aws_mqtt_utf8_validator, NULL))) {
            AWS_LOGF_ERROR(
                AWS_LS_MQTT5_GENERAL,
...
int aws_mqtt5_packet_publish_view_validate(const struct aws_mqtt5_packet_publish_view *publish_view) {
    ...
    if (publish_view->topic.len == 0) {
        ...
        return aws_raise_error(AWS_ERROR_MQTT5_PUBLISH_OPTIONS_VALIDATION);
    } else if (!aws_decode_utf8(publish_view->topic, aws_mqtt_utf8_validator, NULL)) {
        AWS_LOGF_ERROR(
    ...
    if (publish_view->payload_format != NULL) {
        ...
        if (*publish_view->payload_format == AWS_MQTT5_PFI_UTF8) {
            if (!aws_decode_utf8(publish_view->payload, aws_mqtt_utf8_validator, NULL)) {
                AWS_LOGF_ERROR(
                    AWS_LS_MQTT5_GENERAL,
    ...
    if (publish_view->response_topic != NULL) {
        ...
        if (!aws_decode_utf8(*publish_view->response_topic, aws_mqtt_utf8_validator, NULL)) {
            AWS_LOGF_ERROR(
                AWS_LS_MQTT5_GENERAL,
    ...
    if (publish_view->content_type != NULL) {
        ...
        if (!aws_decode_utf8(*publish_view->content_type, aws_mqtt_utf8_validator, NULL)) {
            AWS_LOGF_ERROR(
                AWS_LS_MQTT5_GENERAL,
...
int aws_mqtt5_packet_unsubscribe_view_validate(const struct aws_mqtt5_packet_unsubscribe_view *unsubscribe_view) {
    ...
    for (size_t i = 0; i < unsubscribe_view->topic_filter_count; ++i) {
        ...
        if (!aws_decode_utf8(*topic_filter, aws_mqtt_utf8_validator, NULL)) {
            AWS_LOGF_ERROR(
                AWS_LS_MQTT5_GENERAL,
...
static int s_aws_mqtt5_validate_subscription(
    ...
    if (!aws_decode_utf8(subscription->topic_filter, aws_mqtt_utf8_validator, NULL)) {
        AWS_LOGF_ERROR(
            AWS_LS_MQTT5_GENERAL,
