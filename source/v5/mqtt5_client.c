 ...
 * @return int
 */
int aws_mqtt5_negotiated_settings_reset(
    ...
    negotiated_settings->topic_alias_maximum = 0;

    // Default for Client is QoS 1. Server default is 2.
    // This should only be changed if server returns a 0 in the CONNACK
    // Ask Bret if this is user set anywhere and if so, if it should be added to aws_mqtt5_packet_connect_view
    ...
    if (packet_connect_view->receive_maximum != NULL) {
        if(*packet_connect_view->receive_maximum == 0){
            // It is a Protocol Error to have the value 0. Should this be caught earlier and just once at init?
        }
    ...
    if (packet_connect_view->maximum_packet_size_bytes != NULL) {
        if(*packet_connect_view->maximum_packet_size_bytes == 0){
            // It is a Protocol Error to have the value 0. Should this be caught earlier and just once at init?
        }
    ...
    if (packet_connect_view->topic_alias_maximum != NULL) {
        negotiated_settings->topic_alias_maximum = *packet_connect_view->topic_alias_maximum;
    }
...
int aws_mqtt5_negotiated_settings_apply_connack(
    ...
    if (connack_data->session_expiry_interval != NULL){
        //Discuss with Bret about the way we should use session_expiry_interval. It appears as though 
        //the Server and Client can have different values here and we may not want a singular stored
        //value representing both? Or we can simply use the shorter of the two?
    }

    if (connack_data->receive_maximum != NULL){
        if(*connack_data->receive_maximum == 0){
            // It is a Protocol Error to have the value 0.
        }
        if(*connack_data->receive_maximum < negotiated_settings->receive_maximum){
            negotiated_settings->receive_maximum = *connack_data->receive_maximum;
        }
    }

    //NULL = Maximum QoS of 2 which we don't support yet. Only a value of 0 may require a setting change.
    if (connack_data->maximum_qos != NULL){
        if (connack_data->maximum_qos == 0){
            negotiated_settings->maximum_qos = AWS_MQTT5_QOS_AT_MOST_ONCE;
        }
    }
    ...
    if (connack_data->maximum_packet_size != NULL){
        if(*connack_data->maximum_packet_size == 0){
            // It is a Protocol Error to have the value 0.
        }

        // if negotiated_settings->maximum_packet_size is currently set to 0, it is set to unlimited size
        if(negotiated_settings->maximum_packet_size == 0){
            negotiated_settings->maximum_packet_size = *connack_data->maximum_packet_size;
        } else if (*connack_data->maximum_packet_size < negotiated_settings->maximum_packet_size) {
            negotiated_settings->maximum_packet_size = *connack_data->maximum_packet_size;
        }
    }
    ...
    if (connack_data->topic_alias_maximum != NULL){
        if(*connack_data->topic_alias_maximum < negotiated_settings->topic_alias_maximum){
            negotiated_settings->topic_alias_maximum = *connack_data->topic_alias_maximum;
        }
    } else {
        negotiated_settings->topic_alias_maximum = 0;
    }
