...
struct aws_mqtt5_client {
    ...
    /*
     * A flag that allows external users (the first case being the mqtt3_to_5 adapter) to signal that the connection
     ...
     * completes.  The reconnect will subsequently establish the requested connection using the parameters passed to
     * the 3 API.
     *
     * Rather than try and catch every escape path from CONNECTING, we lazily reset this flag to false when we
     * enter the CONNECTING state.  On a similar note, we only check this flag in the transition to MQTT_CONNECT.
     *
