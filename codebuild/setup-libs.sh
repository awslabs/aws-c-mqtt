...
# build mqtt5 package
cmake -S $CODEBUILD_SRC_DIR -B $CODEBUILD_SRC_DIR/build -DCMAKE_INSTALL_PREFIX=$INSTAL_PATH
cmake --build $CODEBUILD_SRC_DIR/build --target install
ctest $CODEBUILD_SRC_DIR/build
...
endpoint=$(aws secretsmanager get-secret-value --secret-id "unit-test/endpoint" --query "SecretString" | cut -f2 -d":" | cut -f2 -d\")